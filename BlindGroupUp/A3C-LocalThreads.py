...
from simulator.GymBlindGroupUp import GymBGU
...
env = GymBGU(number_of_agents=number_of_agents)
...
with tf.device("/cpu:0"):
    ...
    master_network = AC_Network(state_size, action_size, (number_of_agents - 1) * comm_size,
                                (number_of_agents - 1) * comm_size if spread_messages else comm_size, 'global',
                                   None)  # Generate global network

    workers = []
    # Create worker classes
    for i in range(FLAGS.num_slaves):
        workers.append(Worker(GymBGU(number_of_agents=number_of_agents), i, state_size, action_size, number_of_agents, trainer, model_path,
