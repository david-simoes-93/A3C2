...
with tf.device("/cpu:0"):
    ...
    for i in range(FLAGS.num_slaves):
        workers.append(Worker(GymPursuit(number_of_agents=number_of_agents), i, state_size,
...
with tf.Session() as sess:
    ...
    for worker in workers:
        worker_work = lambda: worker.work(max_episode_length, gamma, sess, coord, saver, FLAGS.max_epis)
