...
from Helper import update_target_graph, discount, get_empty_loss_arrays, adv, gae, gae_0
import numpy as np
...
class Worker:
    ...
    def train_weights_and_get_comm_gradients(self, rollout, sess, gamma, ac_network, bootstrap_value=0):
        ...
        #if len(grads_m[0][1:])==0:
        #    print("len rollout", len(rollout))
        #    for o, m, a, r in zip(observations, mess_received, actions, rewards):
        #        print(o, m, a, r)
        #    print("MESSAGE GRADS\n",grads_m)
    ...
    def input_mloss_to_output_mloss(self, batch_size, mgrad_per_received, comm_map):
        #if batch_size == 0:
        #    print("mgrad",mgrad_per_received)
        #    print("comm map",comm_map)
    ...
    def work(self, max_episode_length, gamma, sess, coord=None, saver=None, max_episodes=None):
        ...
        while not coord.should_stop():
            ...
            self.episode_chase_lengths.append(info["time_chasing"])
            ...
            if self.comm and len(mgrad_per_received[0]) != 0:
                ...
                for i in range(self.number_of_agents):
                    #if len(mgrad_per_sent[i]) == 0:
                    #    print("steps", episode_step_count)
                    #    print(mgrad_per_sent[i])
                    self.apply_comm_gradients(partial_obs[i], partial_mess_rec[i],
                                              sent_message[i], mgrad_per_sent[i], sess, self.local_AC)
            ...
            if episode_count % 5 == 0:
                ...
                mean_chase_length = np.mean(self.episode_chase_lengths[-5:])
                mean_reward = np.mean(self.episode_rewards[-5:])
                mean_value = np.mean(self.episode_mean_values[-5:])

                if self.is_chief and episode_count % 10 == 0:
                    print("length", mean_length, "chase length", mean_chase_length, "reward", mean_reward)
                ...
                summary.value.add(tag='Perf/ChaseLength', simple_value=float(mean_chase_length))  # avg episode length
