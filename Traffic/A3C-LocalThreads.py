...
from Traffic.A3CSlave import Worker
from simulator.GymTraffic import GymTraffic

max_episode_length = 500
gamma = 0.01  # discount rate for advantage estimation and reward discounting
learning_rate = 1e-4
...
# como as comms ao inicio sao so lixo, ele acaba por aprender a ignora-las
# fix? aprender comms e eventualmente resetar os pesos da main network? BUT HOW
...
with tf.device("/cpu:0"):
    ...
    for i in range(FLAGS.num_slaves):
        workers.append(Worker(GymTraffic(number_of_agents=number_of_agents), i, state_size,
